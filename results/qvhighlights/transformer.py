...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_queries=2, num_encoder_layers=6,
        ...
        super().__init__()
        self.args = args
        # TransformerEncoderLayerThin for moment cls token
    ...
    def forward(self, src, mask, query_embed, pos_embed, video_length=None, moment_idx=None, msrc=None, mpos=None, mmask=None,
                ...
                ssrc=None, spos=None, smask=None,
                ssrcd=None, sposd=None, smaskd=None,
...
class V2T_TransformerEncoderLayer(nn.Module):
    ...
    def forward_post(self,
        ...
        # orch.Size([1, 32, 256])
        # torch.Size([75, 32, 256])
        # torch.Size([75, 32, 256])
        # torch.Size([256, 1, 75])
        # torch.Size([32, 75])


        src2, attn_weights = self.self_attn(q, k, value=v, attn_mask=attn_mask,
                              key_padding_mask=src_key_padding_mask[:, :video_length])
        # print(q.shape, k.shape, v.shape, attn_mask.shape, src_key_padding_mask[:, :video_length].shape, src2.shape)
        # print(src2)
        ...
        src = torch.cat([src[:video_length], src2])
        # print(src)
    ...
    def forward_pre(self, src,
        ...
        q, k, v = pos_src[75:], pos_src[:75], src2[:75]
        # print(q.shape) # 100 32 256

        src2, attn_weights = self.self_attn(q, k, value=v, attn_mask=src_key_padding_mask[:, 75:].permute(1, 0),
...
class T2V_TransformerEncoderLayer(nn.Module):
    ...
    def forward_post(self,
        ...
        # print('here')
        # print(attn_weights.shape) # 32 75 25
        # exit(1)
        src2 = src[:video_length] + self.dropout1(src2)
        src3 = self.norm1(src2)
        src3 = self.linear2(self.dropout(self.activation(self.linear1(src3))))
    ...
    def forward_pre(self, src,
        ...
        src2, attn_weights = self.self_attn(q, k, value=v, attn_mask=src_key_padding_mask[:, 1:76].permute(1, 0),
                                            key_padding_mask=src_key_padding_mask[:, 76:], dummy=dummy)
